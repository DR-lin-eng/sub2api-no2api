...
type Group struct {
	...
	// 是否按上下文长度应用模型阶梯价格；默认开启以保持升级前计费行为
	...
	// 蒸馏分组：禁用上游缓存与恢复型重试，并按请求窗口重写 session ID
	IsDistillationGroup bool `json:"is_distillation_group,omitempty"`
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldModelPricing, group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldModelAllowlist, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
		case group.FieldPeakRateEnabled, group.FieldIsExclusive, group.FieldAllowImageGeneration, group.FieldOpenaiForceImageTool, group.FieldAllowBatchImageGeneration, group.FieldImageRateIndependent, group.FieldVideoRateIndependent, group.FieldLongContextPricingEnabled, group.FieldClaudeCodeOnly, group.FieldIsDistillationGroup, group.FieldModelRoutingEnabled, group.FieldMcpXMLInject, group.FieldAllowMessagesDispatch, group.FieldAllowLive, group.FieldRequireOauthOnly, group.FieldRequirePrivacySet, group.FieldProfitControlEnabled:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldIsDistillationGroup:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field is_distillation_group", values[i])
			} else if value.Valid {
				_m.IsDistillationGroup = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("is_distillation_group=")
	builder.WriteString(fmt.Sprintf("%v", _m.IsDistillationGroup))
	builder.WriteString(", ")
