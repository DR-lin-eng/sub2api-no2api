...
func TestSchedulerCacheCandidatePageUsesLastUsedSideKey(t *testing.T) {
	ctx := context.Background()
	cache := newSchedulerCacheUnit(t)
	bucket := service.SchedulerBucket{
		GroupID:  11,
	...
	token, err := cache.CaptureBucketWriteToken(ctx, bucket)
	require.NoError(t, err)
	require.NoError(t, cache.SetCandidateIndex(ctx, bucket, token, []service.Account{account}))
	require.NoError(t, cache.UpdateLastUsed(ctx, map[int64]time.Time{account.ID: latest}))

	page, hit, err := cache.GetCandidatePage(ctx, bucket, 0, 1)
	require.NoError(t, err)
	require.True(t, hit)
	require.Len(t, page.Accounts, 1)
	require.NotNil(t, page.Accounts[0].LastUsedAt)
	require.Equal(t, latest, *page.Accounts[0].LastUsedAt)
}
