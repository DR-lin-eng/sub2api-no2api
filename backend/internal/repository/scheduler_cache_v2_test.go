...
func setSchedulerV2TestCandidateIndex(t *testing.T, cache *schedulerCache, bucket service.SchedulerBucket, accounts []service.Account) {
	t.Helper()
...
func setSchedulerV2TestSnapshot(t *testing.T, cache *schedulerCache, bucket service.SchedulerBucket, accounts []service.Account) {
	t.Helper()
	token, err := cache.CaptureBucketWriteToken(context.Background(), bucket)
	require.NoError(t, err)
...
func TestSchedulerCacheV2_EngineStateAndCandidatePaging(t *testing.T) {
	...
	setSchedulerV2TestCandidateIndex(t, cache, bucket, accounts)
...
func TestSchedulerCacheV2_ReplaceMovesOnlyOneAccountBetweenBuckets(t *testing.T) {
	...
	setSchedulerV2TestCandidateIndex(t, cache, from, []service.Account{account})
	setSchedulerV2TestCandidateIndex(t, cache, to, nil)
...
func TestSchedulerCacheV2_LastUsedUpdatesCandidateOrder(t *testing.T) {
	...
	setSchedulerV2TestCandidateIndex(t, cache, bucket, []service.Account{account1, account2})
...
func TestSchedulerCacheV2_InvalidatesStaleLegacySnapshotsOnRollback(t *testing.T) {
	...
	setSchedulerV2TestSnapshot(t, cache, bucket, []service.Account{account})
...
func TestSchedulerCacheV2_DisableRestoreAndDeleteAccountAcrossBuckets(t *testing.T) {
	...
	setSchedulerV2TestCandidateIndex(t, cache, first, []service.Account{account})
	setSchedulerV2TestCandidateIndex(t, cache, second, []service.Account{account})
...
func TestSchedulerCacheV2_EmptyIndexIsReadyButMissingMetadataIsMiss(t *testing.T) {
	...
	setSchedulerV2TestCandidateIndex(t, cache, bucket, nil)
	page, hit, err := cache.GetCandidatePage(ctx, bucket, 0, 8)
	require.NoError(t, err)
	require.True(t, hit)
	require.True(t, page.Done)
	...
	setSchedulerV2TestCandidateIndex(t, cache, bucket, []service.Account{account})
...
func TestSchedulerCacheV2_RetirementFencesCandidatePublication(t *testing.T) {
	ctx := context.Background()
	cache := newSchedulerV2TestCache(t)
	bucket := service.SchedulerBucket{GroupID: 13, Platform: service.PlatformOpenAI, Mode: service.SchedulerModeSingle}
	account := schedulerV2TestAccount(103, service.PlatformOpenAI, service.AccountTypeAPIKey, 1, nil)

	initialToken, err := cache.CaptureBucketWriteToken(ctx, bucket)
	require.NoError(t, err)
	require.NoError(t, cache.SetCandidateIndex(ctx, bucket, initialToken, []service.Account{account}))
	staleToken, err := cache.CaptureBucketWriteToken(ctx, bucket)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.False(t, hit)
	err = cache.SetCandidateIndex(ctx, bucket, staleToken, []service.Account{account})
	require.ErrorIs(t, err, service.ErrSchedulerBucketRetired)

	reopenedToken, err := cache.ReopenBucket(ctx, bucket)
	require.NoError(t, err)
	require.NoError(t, cache.SetCandidateIndex(ctx, bucket, reopenedToken, []service.Account{account}))
	page, hit, err := cache.GetCandidatePage(ctx, bucket, 0, 8)
	require.NoError(t, err)
	require.True(t, hit)
	require.Equal(t, []int64{account.ID}, candidateAccountIDs(page.Accounts))
}
