...
func TestEnhanceCSPPolicyAllowsHumanVerificationRuntimes(t *testing.T) {
	...
	assert.Equal(t, 1, countDirectiveValue(result, "script-src", TencentCaptchaCDNDomain))
	assert.Equal(t, 1, countDirectiveValue(result, "script-src", TencentCaptchaGlobalDomain))
	assert.Equal(t, 1, countDirectiveValue(result, "script-src", TencentCaptchaGlobalCDN))
	assert.Equal(t, 1, countDirectiveValue(result, "script-src", TencentCaptchaPrehandle))
	assert.Equal(t, 1, countDirectiveValue(result, "connect-src", TencentCaptchaPrehandle))
	assert.Equal(t, 1, countDirectiveValue(result, "frame-src", TencentCaptchaGlobalDomain))
	assert.Equal(t, 1, countDirectiveValue(result, "frame-src", TencentCaptchaPrehandle))
