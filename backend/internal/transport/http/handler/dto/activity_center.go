...
func publicActivityCampaignConfigJSON(campaign *activitycenter.Campaign) string {
	...
	if campaign.Type == activitycenter.CampaignTypeCustom {
		var config activitycenter.ActivityConfig
		if err := json.Unmarshal([]byte(campaign.ConfigJSON), &config); err != nil || config.Custom == nil {
			return "{}"
		}
		out, err := json.Marshal(map[string]any{"custom": config.Custom})
		if err != nil {
			return "{}"
		}
		return string(out)
