...
func TestOpenAIEnsureForwardErrorResponseCompactKeepaliveOnlyWritesResponseFailed(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodPost, EndpointResponses, nil)
...
func TestOpenAIResponsesWebSocket_PassthroughTracksModelPerTurn(t *testing.T) {
	...
	require.InDelta(t, 28e-6, got.logs[0].TotalCost, 1e-12,
		"the Sol turn must use the official GPT-5.6 Sol rate")
	require.InDelta(t, 16e-6, got.logs[1].TotalCost, 1e-12,
		"the Terra turn must use the Terra rate")
}
...
func TestOpenAIResponsesWebSocket_PassthroughKeepsTurnMappingSnapshot(t *testing.T) {
	...
	require.InDelta(t, 16e-6, got.logs[1].TotalCost, 1e-12,
		"the next turn must use the updated channel mapping and Terra rate")
}
