...
func TestSameAccountRetryDelayFor(t *testing.T) {
	requestScopedErr := &service.UpstreamFailoverError{Scope: service.GatewayFailureScopeRequest}
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			require.Equal(t, tt.want, sameAccountRetryDelayFor(requestScopedErr, tt.retryCount))
		})
	}
