...
func TestResponsesToChatCompletionsPreservesXSearchTool(t *testing.T) {
	...
	require.Equal(t, "2026-08-01", chat.Tools[0].FromDate)
	require.Equal(t, "2026-08-10", chat.Tools[0].ToDate)
	require.JSONEq(t, `{"type":"x_search"}`, string(chat.ToolChoice))
}

func TestResponsesToChatCompletionsDropsUndeclaredXSearchToolChoice(t *testing.T) {
