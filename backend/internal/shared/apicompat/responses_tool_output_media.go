...
// and into a following user message. Native Responses providers such as
// DeepSeek require function_call_output.output to be a string, while Codex
// view_image returns input_image parts inside that field.
func LiftResponsesToolOutputMedia(input any) (any, bool) {
	...
	flushMedia := func() {
		...
		rewritten = append(rewritten, map[string]any{"type": "message", "role": "user", "content": content})
		pending = pending[:0]
	}
	...
	for _, rawItem := range items {
		...
		for _, part := range media {
			...
			if imageURL != "" {
				pending = append(pending, responsesToolOutputMedia{callID: callID, imageURL: imageURL})
			}
		}
		changed = true
		rewritten = append(rewritten, item)
