...
type Repository interface {
	...
	ListInflationCampaigns(ctx context.Context, now time.Time) ([]Campaign, error)
	...
	WithCheckinTx(ctx context.Context, campaignID, userID int64, fn func(context.Context) error) error
	CountUserPoolRecordsSince(ctx context.Context, userID, campaignID int64, poolID string, since time.Time) (int64, error)
	CountPrizeRecordsBatch(ctx context.Context, campaignID int64) (map[string]int64, error)
	ListCampaignIssuedCodes(ctx context.Context, campaignID int64) (map[string][]string, error)
...
func (s *Service) ResolveInflatedBalance(ctx context.Context, userID int64, amount float64) (float64, int64, string, string, float64, error) {
	...
	items, err := s.repo.ListInflationCampaigns(ctx, time.Now())
	if err != nil {
		return 0, 0, "", "", 0, err
	}
	if len(items) == 0 {
	...
	if math.IsNaN(credited) || math.IsInf(credited, 0) {
		return 0, 0, "", "", 0, ErrCampaignConfigInvalid
	}
...
func (s *Service) Participate(ctx context.Context, campaignID int64, input ParticipateInput) (*Record, error) {
	...
	if granted != nil {
		s.repo.AfterCommit(ctx, func() { s.rewardGranter.AfterCommit(ctx, input.UserID, *granted) })
	}
	return record, nil
}

func (s *Service) ListUserRecords(ctx context.Context, userID int64, params pagination.PaginationParams, campaignIDs ...int64) ([]Record, *pagination.PaginationResult, error) {
	filters := RecordFilters{UserID: userID}
	if len(campaignIDs) > 0 {
		filters.CampaignID = campaignIDs[0]
	}
...
func (s *Service) PrizeStockStats(ctx context.Context, campaign *Campaign) ([]PrizeStockStat, error) {
	...
	issuedCounts, err := s.repo.CountPrizeRecordsBatch(ctx, campaign.ID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) drawPrize(ctx context.Context, campaignID int64, pool *LotteryPool) (*LotteryPrize, string, error) {
	issuedCounts, err := s.repo.CountPrizeRecordsBatch(ctx, campaignID)
	if err != nil {
		return nil, "", err
	}
	var issuedCodes map[string][]string
	for _, prize := range pool.Prizes {
		if prize.PrizeType == "card" {
			issuedCodes, err = s.repo.ListCampaignIssuedCodes(ctx, campaignID)
			if err != nil {
				return nil, "", err
			}
			break
	...
	for _, prize := range pool.Prizes {
		...
		if prize.PrizeType != "card" && limited && used >= stock {
			continue
		}
		available = append(available, prize)
		availableCodes = append(availableCodes, candidateCode)
		if int64(prize.Weight) > math.MaxInt64-totalWeight {
			return nil, "", ErrCampaignConfigInvalid
		}
...
func normalizeCampaign(c *Campaign) error {
	...
	if c.Type == CampaignTypeInflate || c.Type == CampaignTypeRedeem {
		config, err := parseActivityConfig(c.ConfigJSON)
		if err != nil || validateInflateConfig(config) != nil {
			return ErrCampaignConfigInvalid
		}
...
func validateInflateConfig(config *ActivityConfig) error {
	...
	if err != nil || math.IsNaN(minValue) || math.IsInf(minValue, 0) || minValue < 0 {
		return ErrCampaignConfigInvalid
	}
	maxValue, err := strconv.ParseFloat(strings.TrimSpace(rule.MaxValue), 64)
	if err != nil || math.IsNaN(maxValue) || math.IsInf(maxValue, 0) || maxValue < minValue {
		return ErrCampaignConfigInvalid
	}
	minPct, err := strconv.ParseFloat(strings.TrimSpace(rule.MinInflatePct), 64)
	if err != nil || math.IsNaN(minPct) || math.IsInf(minPct, 0) || minPct < 0 || minPct > 100 {
		return ErrCampaignConfigInvalid
	}
	maxPct, err := strconv.ParseFloat(strings.TrimSpace(rule.MaxInflatePct), 64)
	if err != nil || math.IsNaN(maxPct) || math.IsInf(maxPct, 0) || maxPct < minPct || maxPct > 100 {
		return ErrCampaignConfigInvalid
	}
	if rule.Priority < 0 {
		return ErrCampaignConfigInvalid
...
func validateCheckinConfig(config *ActivityConfig) error {
	...
	if _, err := checkinLocation(config.Checkin); err != nil {
		return ErrCampaignConfigInvalid
	}
	if len(config.Checkin.DailyRewards) != maxDay {
		return ErrCampaignConfigInvalid
	}
	for _, groupID := range config.Checkin.RequiredGroupIDs {
		if groupID <= 0 {
			return ErrCampaignConfigInvalid
		}
	}
	...
	for _, reward := range config.Checkin.DailyRewards {
		...
		case "balance":
			if value, err := strconv.ParseFloat(reward.Value, 64); err != nil || math.IsNaN(value) || math.IsInf(value, 0) || value <= 0 {
				return ErrCampaignConfigInvalid
			}
		case "concurrency":
		...
		case "subscription":
			if days, err := strconv.Atoi(reward.Value); err != nil || days <= 0 || reward.RewardGroupID <= 0 {
				return ErrCampaignConfigInvalid
			}
		default:
			return ErrCampaignConfigInvalid
