...
func (s *Service) Checkin(ctx context.Context, campaignID, userID int64) (*Record, *CheckinStatus, error) {
	...
	err := s.repo.WithParticipationTx(ctx, campaignID, func(txCtx context.Context) error {
		...
		cycleDay := 1
		if status.LastCheckinDate != nil {
			if status.LastCheckinDate.Format("2006-01-02") == now.AddDate(0, 0, -1).Format("2006-01-02") {
				cycleDay = status.StreakDays + 1
			}
		}
		if cycleDay > CheckinCycleDays(config.CycleType) {
			cycleDay = 1
		}
		...
		grant = &g
		payload, _ := json.Marshal(map[string]any{"day": cycleDay, "streak_days": cycleDay, "reward_type": reward.RewardType, "value": reward.Value, "label": reward.Label})
		checkin := &CheckinRecord{CampaignID: campaignID, UserID: userID, CheckinDate: time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, time.UTC), CycleNo: 1, CycleDay: cycleDay, StreakDays: cycleDay, RewardType: reward.RewardType, RewardValue: reward.Value, RewardStatus: "granted", RewardPayloadJSON: string(payload)}
	...
	if grant != nil {
		s.rewardGranter.AfterCommit(ctx, userID, *grant)
	}
