...
func (r *fakeCampaignRepo) ListInflationCampaigns(ctx context.Context, now time.Time) ([]Campaign, error) {
	items, err := r.ListVisible(ctx, now)
	if err != nil {
		return nil, err
	}
	out := make([]Campaign, 0)
...
func (r *fakeCampaignRepo) WithCheckinTx(ctx context.Context, _, _ int64, fn func(context.Context) error) error {
	return fn(ctx)
}
func (r *fakeCampaignRepo) CountPrizeRecordsBatch(_ context.Context, campaignID int64) (map[string]int64, error) {
	out := make(map[string]int64)
	for _, v := range r.records {
		if v.CampaignID == campaignID && v.PrizeID != "" {
			out[v.PrizeID]++
		}
	}
	return out, nil
}
func (r *fakeCampaignRepo) ListCampaignIssuedCodes(ctx context.Context, campaignID int64) (map[string][]string, error) {
	out := make(map[string][]string)
	for _, v := range r.records {
		if v.CampaignID == campaignID {
			codes, err := r.ListPrizeIssuedCodes(ctx, campaignID, v.PrizeID)
			if err != nil {
				return nil, err
			}
			out[v.PrizeID] = codes
...
func (r *fakeCampaignRepo) AfterCommit(_ context.Context, fn func()) { fn() }

func TestCampaignRecordsFilterBeforePaginationAndPreserveOwnership(t *testing.T) {
	r := newFakeCampaignRepo()
	r.records = []Record{{UserID: 1, CampaignID: 10}, {UserID: 2, CampaignID: 10}, {UserID: 1, CampaignID: 20}}
	records, _, err := NewService(r).ListUserRecords(context.Background(), 1, pagination.PaginationParams{Page: 1, PageSize: 20}, 10)
	require.NoError(t, err)
	require.Len(t, records, 1)
	require.EqualValues(t, 1, records[0].UserID)
	require.EqualValues(t, 10, records[0].CampaignID)
}
