...
func TestQualityConversationRepositoryRoundTrip(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO account_quality_runs").WithArgs(id, int64(1), "", "", "ready", "", float64(0), now, now, int64(0), "", []byte(nil), []byte(nil), "", qualityDetailsArg{}, []byte(nil), "").WillReturnResult(sqlmock.NewResult(0, 1))
	...
	mock.ExpectQuery("SELECT id::text, account_id").WithArgs(now, 200).WillReturnRows(sqlmock.NewRows([]string{"id", "account_id", "model", "effort", "status", "label", "confidence", "started_at", "finished_at", "latency_ms", "model_version", "error_message", "probe_details", "has_preview", "preview_format"}).AddRow(id, 1, "", "", "ready", "", 0, now, now, 0, "", "", details, false, ""))
