...
func (s *HTTPUpstreamSuite) TestVirtualClientKeySharesOnlyMatchingOpenAIPrincipalPoolKey() {
	first := buildCacheKeyWithVirtualClientKey(config.ConnectionPoolIsolationAccountProxy, "direct", 101, upstreamProtocolModeDefault, "chatgpt:principal")
	second := buildCacheKeyWithVirtualClientKey(config.ConnectionPoolIsolationAccountProxy, "direct", 202, upstreamProtocolModeDefault, "chatgpt:principal")
	other := buildCacheKeyWithVirtualClientKey(config.ConnectionPoolIsolationAccountProxy, "direct", 202, upstreamProtocolModeDefault, "chatgpt:other")
	local := buildCacheKeyWithVirtualClientKey(config.ConnectionPoolIsolationAccountProxy, "direct", 202, upstreamProtocolModeDefault, "")
	require.Equal(s.T(), first, second, "same virtual principal should retain one socket pool across local failover records")
	require.NotEqual(s.T(), first, other)
	require.Equal(s.T(), "account:202|proxy:direct", local)
}

func (s *HTTPUpstreamSuite) TestVirtualClientRouteRotationEvictsOnlyMatchingPrincipal() {
	oldRoute := platformegress.IPv6PoolRoute("2001:db8::10", 1, 1, false)
	newRoute := platformegress.IPv6PoolRoute("2001:db8::11", 1, 2, false)
	svc := &httpUpstreamService{clients: make(map[string]*upstreamClientEntry)}
	svc.clients["shared-old"] = &upstreamClientEntry{accountID: 101, virtualClientKey: "chatgpt:shared", routeKey: oldRoute.CacheKey(), routeMode: platformegress.ModeIPv6Pool}
	svc.clients["other-old"] = &upstreamClientEntry{accountID: 202, virtualClientKey: "chatgpt:other", routeKey: oldRoute.CacheKey(), routeMode: platformegress.ModeIPv6Pool}
	svc.evictStaleAccountRoutesLocked(303, "chatgpt:shared", newRoute)
	require.NotContains(s.T(), svc.clients, "shared-old")
	require.Contains(s.T(), svc.clients, "other-old")
}
