...
func transientDatabaseInitializationError(code string) error {
...
func TestIsTransientDatabaseInitializationError(t *testing.T) {
	tests := []struct {
		...
		{name: "postgres is starting", err: transientDatabaseInitializationError("57P03"), want: true},
		{name: "connection failure", err: fmt.Errorf("wrapped: %w", transientDatabaseInitializationError("08006")), want: true},
		{name: "authentication failure", err: transientDatabaseInitializationError("28P01"), want: false},
...
func TestInitializeDatabaseWithRetryEventuallySucceeds(t *testing.T) {
	...
	err := initializeDatabaseWithRetryWithWait(context.Background(), func(context.Context) error {
		attempts++
		if attempts <= 3 {
			return transientDatabaseInitializationError("57P03")
		}
		return nil
...
func TestInitializeDatabaseWithRetryFailsFastForPermanentError(t *testing.T) {
	attempts := 0
	waitCalled := false
	permanentErr := transientDatabaseInitializationError("28P01")
	err := initializeDatabaseWithRetryWithWait(context.Background(), func(context.Context) error {
		attempts++
		return permanentErr
...
func TestInitializeDatabaseWithRetryReturnsLastErrorAfterLimit(t *testing.T) {
	attempts := 0
	lastErr := transientDatabaseInitializationError("08006")
	var delays []time.Duration
	err := initializeDatabaseWithRetryWithWait(context.Background(), func(context.Context) error {
		attempts++
		return lastErr
