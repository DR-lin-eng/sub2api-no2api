...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	if openAITiming.Valid {
		var timing openaitiming.Metrics
		if err := json.Unmarshal([]byte(openAITiming.String), &timing); err == nil {
			log.OpenAITiming = &timing
		}
	}
