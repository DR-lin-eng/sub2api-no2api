...
var imageStorageResolutionBenchmarkSink bool

func BenchmarkImageStorageSettingServiceResolveCached(b *testing.B) {
	svc := &ImageStorageSettingService{}
	svc.resolution.Store(&imageStorageResolution{enabled: true})
	b.ReportAllocs()
	for range b.N {
		_, imageStorageResolutionBenchmarkSink = svc.resolve()
	}
}
