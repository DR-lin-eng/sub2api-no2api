...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	resolution, err := svc.calculateOpenAIRecordUsageCostWithResolution(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 3.0, 1.0, 1.0, 2.0, UsageTokens{}, "", false)
	require.NoError(t, err)
	require.NotNil(t, resolution.Cost)
	...
	resolution, err = svc.calculateOpenAIRecordUsageCostWithResolution(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{}, "", false)
	require.NoError(t, err)
	require.NotNil(t, resolution.Cost)
	cost = resolution.Cost
	...
	_, err = svc.calculateOpenAIRecordUsageCostWithResolution(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10}, "", false)
	require.Error(t, err)
}
