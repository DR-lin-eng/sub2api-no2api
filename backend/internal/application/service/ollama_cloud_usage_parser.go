...
func parseOllamaCloudUsageHTML(body []byte) (*OllamaCloudUsageData, error) {
	...
	data.Plan = valueBesideIndexedLabel(labels, []string{"cloud usage"}, 80)
	if data.Plan == "" {
		data.Plan = valueBesideIndexedLabel(labels, []string{"plan", "subscription"}, 80)
	}
	data.FiveHour = parseIndexedOllamaUsageWindow(labels, ollamaFiveHourUsageAliases)
	data.SevenDay = parseIndexedOllamaUsageWindow(labels, ollamaSevenDayUsageAliases)
	data.Balance = valueBesideIndexedLabel(labels, []string{"balance remaining"}, 80)
	if data.Balance == "" {
		if match := ollamaBalancePattern.FindStringSubmatch(pageText); len(match) == 2 {
			data.Balance = strings.Join(strings.Fields(match[1]), "")
		}
	}
	data.Models = parseIndexedOllamaModels(doc, labels)
...
func parseIndexedOllamaUsageWindow(labels *ollamaLabelIndex, aliases []string) *OllamaCloudUsageWindow {
	label := labels.find(aliases)
	if label == nil {
		return nil
	}
...
func valueBesideIndexedLabel(labels *ollamaLabelIndex, aliases []string, maxLen int) string {
	label := labels.find(aliases)
	if label == nil {
		return ""
	}
...
func findLabelElement(root *html.Node, aliases []string) *html.Node {
	return buildOllamaLabelIndex(root).find(aliases)
}

const (
	ollamaLabelTextMaxBytes          = 192
	ollamaLabelAncestorHopLimit      = 16
	ollamaLabelContainerPerTextLimit = 6
)

type ollamaLabelCandidate struct {
	node     *html.Node
	text     string
	overflow bool
}

// ollamaLabelIndex collects bounded text for nearby label containers in one
// DOM pass. Each text node contributes to a fixed number of ancestors, so a
// deeply nested settings page cannot trigger one recursive subtree walk per
// container.
type ollamaLabelIndex struct {
	candidates []ollamaLabelCandidate
}

func buildOllamaLabelIndex(root *html.Node) *ollamaLabelIndex {
	index := &ollamaLabelIndex{}
	if root == nil {
		return index
	}
	candidateByNode := make(map[*html.Node]int)
	...
	visit = func(node *html.Node) {
		if node.Type == html.ElementNode {
			...
			if isOllamaParserContainer(node.Data) {
				candidateByNode[node] = len(index.candidates)
				index.candidates = append(index.candidates, ollamaLabelCandidate{node: node})
			}
		}
		if node.Type == html.TextNode {
			...
			if text != "" {
				containers := 0
				for ancestor, hops := node.Parent, 0; ancestor != nil && hops < ollamaLabelAncestorHopLimit && containers < ollamaLabelContainerPerTextLimit; ancestor, hops = ancestor.Parent, hops+1 {
					candidateIndex, ok := candidateByNode[ancestor]
					if !ok {
						continue
					}
					containers++
					index.appendText(candidateIndex, text)
				}
			}
		}
...
func (i *ollamaLabelIndex) appendText(candidateIndex int, value string) {
	candidate := &i.candidates[candidateIndex]
	...
	if len(candidate.text)+additional > ollamaLabelTextMaxBytes {
...
func (i *ollamaLabelIndex) find(aliases []string) *html.Node {
	if i == nil {
		return nil
	}
	...
	for index := range i.candidates {
		candidate := &i.candidates[index]
		if candidate.overflow || candidate.text == "" {
			continue
		}
		...
		for _, alias := range normalizedAliases {
			if (text == alias || strings.HasPrefix(text, alias+" ") || strings.HasPrefix(text, alias+":")) && len(text) < bestLen {
				best, bestLen = candidate.node, len(text)
			}
		}
	}
	return best
...
func parseIndexedOllamaModels(root *html.Node, labels *ollamaLabelIndex) []OllamaCloudUsageModel {
	...
	if len(models) == 0 {
		heading := labels.find([]string{"models", "available models"})
