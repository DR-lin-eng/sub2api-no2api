...
func (s *GrokOAuthService) GenerateAuthURL(ctx context.Context, proxyID *int64, redirectURI string) (*GrokAuthURLResult, error) {
	...
	if err := s.sessionStore.SetContext(ctx, sessionID, &xai.OAuthSession{
		State:         state,
...
func (s *GrokOAuthService) ExchangeCode(ctx context.Context, input *GrokExchangeCodeInput) (*GrokTokenInfo, error) {
	...
	if err != nil {
		return nil, infraerrors.Newf(http.StatusServiceUnavailable, "GROK_OAUTH_SESSION_STORE_FAILED", "failed to load oauth session: %v", err)
	}
	if !ok {
		return nil, infraerrors.New(http.StatusBadRequest, "GROK_OAUTH_SESSION_NOT_FOUND", "session not found or expired")
	}
	defer func() { _ = s.sessionStore.DeleteContext(context.Background(), input.SessionID) }()
...
func (s *GrokOAuthService) RefreshAccountToken(ctx context.Context, account *Account) (*GrokTokenInfo, error) {
	...
	// The refreshed access-token JWT is authoritative. Preserve stored values
	// only for opaque tokens or responses without the corresponding claim.
