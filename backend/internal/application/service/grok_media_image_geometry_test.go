...
func TestApplyGrokImagineImageGeometryMapsOpenAISize(t *testing.T) {
	...
	out, err := applyGrokImagineImageGeometry([]byte(`{"model":"grok-imagine-image","prompt":"hi","size":"1152x1536"}`))
...
func TestParseGrokMediaRequestSeparatesImageResolutionFromVideoResolution(t *testing.T) {
	t.Parallel()

	info := ParseGrokMediaRequest("application/json", []byte(`{"model":"grok-imagine-image","resolution":"2K","aspect_ratio":"16:9"}`))
