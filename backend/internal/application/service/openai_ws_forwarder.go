...
func newOpenAIWSCurrentTurnFailoverError(cause error, retryPayload []byte) error {
	return &openAIWSCurrentTurnFailoverError{cause: cause, retryPayload: append([]byte(nil), retryPayload...)}
}

// OpenAIWSCurrentTurnRetryPayload returns an isolated payload that may be
// retried on a replacement account without replaying the initial turn.
