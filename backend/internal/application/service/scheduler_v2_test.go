...
func (c *schedulerV2CacheStub) CaptureBucketWriteToken(_ context.Context, bucket SchedulerBucket) (SchedulerBucketWriteToken, error) {
	return SchedulerBucketWriteToken{Bucket: bucket, Epoch: 1}, nil
}

func (c *schedulerV2CacheStub) SetSnapshot(_ context.Context, bucket SchedulerBucket, _ SchedulerBucketWriteToken, accounts []Account) error {
	c.legacySnapshot = accountPointers(accounts)
	return nil
}

func (c *schedulerV2CacheStub) RetireBucket(_ context.Context, bucket SchedulerBucket) error {
	delete(c.candidates, bucket.String())
	delete(c.ready, bucket.String())
	return nil
}

func (c *schedulerV2CacheStub) ReopenBucket(_ context.Context, bucket SchedulerBucket) (SchedulerBucketWriteToken, error) {
	return SchedulerBucketWriteToken{Bucket: bucket, Epoch: 1}, nil
}

func (c *schedulerV2CacheStub) TryAcquireGroupLifecycleLease(_ context.Context, groupID int64, _ time.Duration) (SchedulerGroupLifecycleLease, bool, error) {
	return SchedulerGroupLifecycleLease{GroupID: groupID, OwnerToken: "test"}, true, nil
}

func (c *schedulerV2CacheStub) ReleaseGroupLifecycleLease(context.Context, SchedulerGroupLifecycleLease) error {
	return nil
}
...
func (c *schedulerV2CacheStub) SetCandidateIndex(_ context.Context, bucket SchedulerBucket, token SchedulerBucketWriteToken, accounts []Account) error {
...
func setSchedulerV2StubIndex(t *testing.T, cache *schedulerV2CacheStub, bucket SchedulerBucket, accounts []Account) {
	t.Helper()
	token, err := cache.CaptureBucketWriteToken(context.Background(), bucket)
	require.NoError(t, err)
	require.NoError(t, cache.SetCandidateIndex(context.Background(), bucket, token, accounts))
}
...
func TestSchedulerSnapshotV2_NeverReadsLegacySnapshotOrFallback(t *testing.T) {
	...
	setSchedulerV2StubIndex(t, cache, bucket, []Account{account})
...
func TestSchedulerSnapshotV2_PaginatesPastBlockedAndExcludedCandidates(t *testing.T) {
	...
	setSchedulerV2StubIndex(t, cache, bucket, accounts)
...
func TestSchedulerSnapshotV2_EngineStateReadsAreLocallyBounded(t *testing.T) {
	cache := newSchedulerV2CacheStub()
	cache.state = SchedulerEngineState{
		Engine:         SchedulerEngineV2,
		Status:         SchedulerEngineStatusActive,
	...
	for range 100 {
		require.True(t, svc.SchedulerEngineState(context.Background()).V2Enabled())
	}

	require.Equal(t, 1, cache.engineReads)
}
...
func TestSchedulerV2_PaginatesPastModelIncompatibleWindow(t *testing.T) {
	...
	selected, err := gateway.selectAccountForModelWithExclusions(
		context.Background(), nil, PlatformOpenAI, "", "gpt-target", nil, false, 0, "", false,
	)
	require.NoError(t, err)
...
func TestSchedulerSnapshotV2_RecoversMissingRedisStateFromPersistedTarget(t *testing.T) {
	...
	svc := NewSchedulerSnapshotService(cache, nil, accountRepo, nil, &config.Config{RunMode: config.RunModeSimple})
...
func TestSchedulerV2_OldAndNewOpenAICompatibleSelectionAcrossTypes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.platform+"/"+tc.accountType, func(t *testing.T) {
			...
			cache.state = SchedulerEngineState{Engine: SchedulerEngineLegacy, Status: SchedulerEngineStatusDisabled}
			legacy, err := gateway.selectAccountForModelWithExclusions(context.Background(), nil, tc.platform, "", "", nil, false, 0, "", false)
			require.NoError(t, err)
			cache.state = SchedulerEngineState{Engine: SchedulerEngineV2, Status: SchedulerEngineStatusActive}
			v2, err := gateway.selectAccountForModelWithExclusions(context.Background(), nil, tc.platform, "", "", nil, false, 0, "", false)
			require.NoError(t, err)
