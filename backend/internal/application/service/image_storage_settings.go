...
type imageStorageResolution struct {
...
func (s *ImageStorageSettingService) resolve() (*ImageResultUploader, bool) {
	...
	if current := s.resolution.Load(); current != nil {
		return current.uploader, current.enabled
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	if current := s.resolution.Load(); current != nil {
	...
	resolved := &imageStorageResolution{}
