...
func TestGrokMediaGenerationEligibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "cached billing map remains eligible", account: &Account{Platform: PlatformGrok, Type: AccountTypeOAuth, Extra: map[string]any{grokBillingExtraKey: map[string]any{"status_code": float64(http.StatusOK), "weekly_status_code": float64(http.StatusOK), "monthly_status_code": float64(http.StatusOK), "usage_percent": 12.5}}}, want: true, wantReason: "eligible"},
		{name: "cached weekly forbidden is rejected", account: &Account{Platform: PlatformGrok, Type: AccountTypeOAuth, Extra: map[string]any{grokBillingExtraKey: map[string]any{"status_code": float64(http.StatusOK), "weekly_status_code": float64(http.StatusForbidden), "monthly_status_code": float64(http.StatusOK)}}}, want: false, wantReason: "billing_forbidden"},
