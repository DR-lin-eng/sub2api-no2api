...
func TestAdaptGrokResponsesClientToolsSkipsOrdinaryFunctionBody(t *testing.T) {
	body := []byte(`{"tools":[{"type":"function","name":"lookup"}],"input":"unchanged"}`)

	adapted, mapping, err := adaptGrokResponsesClientTools(body)
	require.NoError(t, err)
	require.Equal(t, body, adapted)
	require.False(t, hasGrokResponsesClientToolMapping(mapping))
}

func BenchmarkAdaptGrokResponsesClientTools(b *testing.B) {
	largeInput := strings.Repeat("x", 256*1024)
	ordinaryBody, err := json.Marshal(map[string]any{
		"model": "grok-4.5",
	...
	customBody, err := json.Marshal(map[string]any{
		"model": "grok-4.5",
		"tools": []any{map[string]any{
			"type": "custom",
			"name": "apply_patch",
	...
	b.Run("ordinary_function_fast_path", func(b *testing.B) {
		b.ReportAllocs()
		for range b.N {
			if _, _, err := adaptGrokResponsesClientTools(ordinaryBody); err != nil {
				b.Fatal(err)
			}
		}
	})
	b.Run("ordinary_function_full_decode_reference", func(b *testing.B) {
		b.ReportAllocs()
		for range b.N {
			decoder := json.NewDecoder(bytes.NewReader(ordinaryBody))
			decoder.UseNumber()
			var requestBody map[string]any
			if err := decoder.Decode(&requestBody); err != nil {
				b.Fatal(err)
			}
			if _, _, err := apicompat.AdaptResponsesClientTools(requestBody); err != nil {
				b.Fatal(err)
			}
		}
	})
	b.Run("custom_tool_adaptation", func(b *testing.B) {
		b.ReportAllocs()
		for range b.N {
			if _, _, err := adaptGrokResponsesClientTools(customBody); err != nil {
				b.Fatal(err)
			}
		}
	})
