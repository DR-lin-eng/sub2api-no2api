...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	if account.IsOpenCodeGo() {
		...
		case APIProtocolResponses:
			// Continue through the Messages-to-Responses bridge below.
		default:
	...
	if account.Type == AccountTypeAPIKey && !account.IsOpenCodeGo() && !openai_compat.ShouldUseResponsesAPI(account.Extra) {
		return s.forwardAnthropicViaRawChatCompletions(ctx, c, account, body, defaultMappedModel)
	}
	...
	if !distillation && promptCacheKey == "" && shouldAutoInjectPromptCacheKeyForCompat(upstreamModel) {
	...
	if distillation {
		responsesBody = stripDistillationCacheFields(responsesBody)
	}
	...
	if resp.StatusCode >= 400 {
		...
		if !distillation && !agentIdentityTaskRecoveryWasTried(ctx) && s.isAgentIdentityAccount(ctx, account) && isAgentIdentityTaskInvalidHTTPResponse(resp.StatusCode, respBody) {
		...
		if !distillation && anthropicCompactRequest && anthropicCompactModelMapped &&
		...
		if !distillation && previousResponseID != "" && (isOpenAICompatPreviousResponseNotFound(resp.StatusCode, upstreamMsg, respBody) || isOpenAICompatPreviousResponseUnsupported(resp.StatusCode, upstreamMsg, respBody)) {
		...
		if !distillation && account.Platform == PlatformGrok &&
...
func (s *OpenAIGatewayService) handleAnthropicCompactStreamingResponse(
	...
	finalResponse, usage, acc, err := s.readOpenAICompatBufferedTerminal(resp, "openai messages compact buffered", requestID, c)
	if err != nil {
		return nil, err
	}
...
func (s *OpenAIGatewayService) handleAnthropicBufferedStreamingResponse(
	...
	finalResponse, usage, acc, err := s.readOpenAICompatBufferedTerminal(resp, "openai messages buffered", requestID, c)
	if err != nil {
...
// openAICompatBufferedReadError marks failures that happened while reading an
// upstream SSE body before a terminal response was available. The shared
// buffered reader is used by both Chat Completions and Messages; callers decide
// independently whether replaying the request is safe for their endpoint.
type openAICompatBufferedReadError struct {
	cause error
}

func (e *openAICompatBufferedReadError) Error() string {
	if e == nil || e.cause == nil {
		return "upstream buffered response read failed"
	}
	return e.cause.Error()
}

func (e *openAICompatBufferedReadError) Unwrap() error {
	if e == nil {
		return nil
	}
	return e.cause
...
func (s *OpenAIGatewayService) readOpenAICompatBufferedTerminal(
	...
	var timingContext *gin.Context
	if len(contexts) > 0 {
		timingContext = contexts[0]
	}
