...
// extractOpenAIImagesModelText collects bounded textual output from an image
// request. A text-only response means the image tool did not execute; callers
// classify safety refusals separately from transient capability failures.
			...
			if len(finalResults) == 0 {
				...
				if textFallbackErr != nil {
					...
					if !retryable && !clientDisconnected {
						writeStreamEvent("error", buildOpenAIImagesStreamErrorBodyFromUpstream(textFallbackErr))
					}
