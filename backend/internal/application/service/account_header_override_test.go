...
func TestIsHeaderOverrideEligible(t *testing.T) {
	tests := []struct {
		...
		{"kimi apikey", "kimi", AccountTypeAPIKey, true},
		{"zhipu apikey", "zhipu", AccountTypeAPIKey, true},
		{"deepseek apikey", "deepseek", AccountTypeAPIKey, true},
		{"anthropic oauth", PlatformAnthropic, AccountTypeOAuth, false},
		{"openai oauth", PlatformOpenAI, AccountTypeOAuth, false},
		{"kimi oauth", "kimi", AccountTypeOAuth, false},
		{"zhipu oauth", "zhipu", AccountTypeOAuth, false},
		{"deepseek oauth", "deepseek", AccountTypeOAuth, false},
...
func TestNormalizeHeaderOverrideCredentials(t *testing.T) {
	...
	t.Run("rejects blocked headers", func(t *testing.T) {
		for _, name := range []string{
			...
			"connection", "accept-encoding", "Sec-WebSocket-Key", "session_id", "session-id", "thread_id", "thread-id",
