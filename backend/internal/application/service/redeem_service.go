...
func (s *RedeemService) Redeem(ctx context.Context, userID int64, code string) (*RedeemCode, error) {
	...
	originalRedeemValue := redeemCode.Value
	amount := originalRedeemValue
	var inflationCampaignID int64
	var inflationTitle, inflationType string
	var inflationPct float64
	if redeemCode.Type == RedeemTypeBalance && amount > 0 && s.inflationResolver != nil {
	...
	if redeemCode.Type == RedeemTypeBalance && amount != redeemCode.Value {
		if writer, ok := s.redeemRepo.(RedeemCodeUsageValueWriter); ok {
			...
			return nil, errors.New("redeem repository cannot persist inflated value")
		}
	}
	if err := use(); err != nil {
	...
	if redeemCode.Type == RedeemTypeBalance && redeemCode.Value > 0 {
		s.tryAccrueAffiliateRebateForRedeem(ctx, userID, originalRedeemValue)
	}
	...
	// The response reports this redemption credit; reusable codes retain their face value in storage.
	if redeemCode.Type == RedeemTypeBalance {
		redeemCode.Value = amount
	}
