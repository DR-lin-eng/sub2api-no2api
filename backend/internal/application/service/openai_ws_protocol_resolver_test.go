...
func TestOpenAIWSProtocolResolver_Resolve(t *testing.T) {
	...
	t.Run("v2关闭时回退v1", func(t *testing.T) {
		cfg := cloneOpenAIWSProtocolTestConfig(baseCfg)
...
func cloneOpenAIWSProtocolTestConfig(base *config.Config) *config.Config {
	clone := &config.Config{}
	if base != nil {
		clone.Gateway.OpenAIWS = base.Gateway.OpenAIWS
	}
	return clone
}
