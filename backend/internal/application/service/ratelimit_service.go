...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	if statusCode == http.StatusTooManyRequests && account.Platform == PlatformAnthropic {
		creditsRequired := s.persistAnthropicFableCreditsRequired(ctx, account, headers, responseBody, firstRequestedModel(requestedModel))
		...
		if creditsRequired || fableLimited {
			return false
		}
	}
...
func (s *RateLimitService) handleOpenAI403(ctx context.Context, account *Account, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	// HTML 403 pages are emitted by proxies or CDNs before OpenAI evaluates
	// credentials. Keep request failover intact, but do not turn an endpoint or
	// network rejection into account-level state.
...
func parseOpenCodeGoUsageLimitResetDuration(message string) time.Duration {
	...
	if !parsedPart || !openCodeGoUsageLimitDurationHasValidTerminator(remainder) {
		return 0
	}
	return total
}

func openCodeGoUsageLimitDurationHasValidTerminator(remainder string) bool {
	remainder = strings.TrimSpace(remainder)
	if remainder == "" {
		return true
	}
