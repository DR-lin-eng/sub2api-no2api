...
func TestObserveRelayTurnResponseModelCapsUntrustedModelNames(t *testing.T) {
	t.Parallel()

	turn := &relayTurnTiming{}
	model := strings.Repeat("模", relayResponseModelMaxLength+25)

	observeRelayTurnResponseModel(turn, model, true)

	require.Len(t, []rune(turn.terminalResponseModel), relayResponseModelMaxLength)
}
