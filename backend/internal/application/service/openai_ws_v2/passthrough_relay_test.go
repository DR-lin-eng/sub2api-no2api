...
func TestRelay_BasicRelayAndUsage(t *testing.T) {
	...
	require.Nil(t, result.FirstTokenMs, "a usage-only terminal event is not a token event")
...
func TestRelay_NoSemanticOutputTerminalSequence_FirstTokenMsNil(t *testing.T) {
	...
	for _, terminalEvent := range []string{"response.completed", "response.done"} {
		...
		t.Run(terminalEvent, func(t *testing.T) {
			...
			clientConn := newPassthroughTestFrameConn(nil, false)
			upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{
				{msgType: coderws.MessageText, payload: []byte(`{"type":"response.created","response":{"id":"resp_no_output"}}`)},
				{msgType: coderws.MessageText, payload: []byte(`{"type":"response.in_progress","response":{"id":"resp_no_output"}}`)},
				{msgType: coderws.MessageText, payload: []byte(`{"type":"response.content_part.done","response_id":"resp_no_output"}`)},
				{msgType: coderws.MessageText, payload: []byte(`{"type":"response.output_item.done","response_id":"resp_no_output"}`)},
				{msgType: coderws.MessageText, payload: []byte(`{"type":"` + terminalEvent + `","response":{"id":"resp_no_output","usage":{"input_tokens":2,"output_tokens":0}}}`)},
			}, true)
