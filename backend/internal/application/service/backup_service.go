...
const (
	...
	backupOperationTimeout     = 6 * time.Hour
	backupScheduledLockTTL     = backupOperationTimeout + 5*time.Minute
)
...
type BackupObjectStore interface {
	UploadFileRange(ctx context.Context, key, filePath string, offset, sizeBytes int64, contentType string) error
...
// BackupPart describes one contiguous range of the gzip archive stored as an object.
...
// BackupDownloadResponse preserves the legacy single URL while supporting split archives.
...
type BackupService struct {
	...
	partSizeBytes int64              // production default is 4 GiB; tests may inject a smaller value
}
...
func (s *BackupService) uploadBackupArchive(ctx context.Context, record *BackupRecord, objectStore BackupObjectStore, cfg *BackupS3Config, archivePath string) error {
	...
	if info.Size() <= partSize {
		record.Parts = nil
		if err := objectStore.UploadFileRange(ctx, record.S3Key, archivePath, 0, info.Size(), "application/gzip"); err != nil {
	...
	plans, err := planBackupFileParts(archivePath, partSize, func(index int) string {
		return s.buildBackupPartKey(partRoot, index)
	})
	if err != nil {
		return fmt.Errorf("plan backup archive parts: %w", err)
	}
	record.S3Key = ""
	record.Parts = make([]BackupPart, 0, len(plans))
	for _, plan := range plans {
		record.Parts = append(record.Parts, plan.Part)
	}
	if err := s.saveRecord(ctx, record); err != nil {
	...
	for _, plan := range plans {
		part := plan.Part
		if err := objectStore.UploadFileRange(ctx, part.S3Key, archivePath, plan.Offset, part.SizeBytes, "application/octet-stream"); err != nil {
			cleanupCtx, cleanupCancel := context.WithTimeout(context.Background(), backupObjectCleanupTimeout)
			cleanupErr := deleteBackupObjectKeys(cleanupCtx, objectStore, record)
			cleanupCancel()
...
func (s *BackupService) executeRestore(record *BackupRecord, objectStore BackupObjectStore) {
	...
	if strings.TrimSpace(record.S3Key) == "" {
		record.RestoreStatus = "failed"
		record.RestoreError = "backup object key is empty"
		_ = s.saveRecord(context.Background(), record)
		return
	}
...
func (s *BackupService) downloadBackupParts(ctx context.Context, objectStore BackupObjectStore, parts []BackupPart) (path string, err error) {
	...
	for i, part := range ordered {
		if part.Index != i+1 || strings.TrimSpace(part.S3Key) == "" || part.SizeBytes <= 0 {
	...
	for _, part := range ordered {
		...
		hash := sha256.New()
		written, copyErr := io.Copy(io.MultiWriter(archive, hash), io.LimitReader(body, part.SizeBytes+1))
		closeErr := body.Close()
		if copyErr != nil {
			cleanup()
...
func (s *BackupService) DeleteBackup(ctx context.Context, backupID string) error {
	...
	// Keep the record when object deletion is incomplete so the operation can be retried.
...
func (s *BackupService) GetBackupDownloadURL(ctx context.Context, backupID string) (BackupDownloadResponse, error) {
	...
	if len(record.Parts) > 0 {
		...
		for i, part := range parts {
			...
			url, presignErr := objectStore.PresignURL(ctx, part.S3Key, time.Hour)
			if presignErr != nil {
			...
			download.Parts = append(download.Parts, BackupDownloadPart{
				Index: part.Index, SizeBytes: part.SizeBytes, URL: url,
			})
		}
		return download, nil
	}
...
func (s *BackupService) cleanupOldBackups(ctx context.Context, schedule *BackupScheduleConfig) error {
	...
	if len(toDelete) > 0 {
		// Preserve a stable newest-first order after reinserting failed deletions.
		sort.Slice(toKeep, func(i, j int) bool { return toKeep[i].StartedAt > toKeep[j].StartedAt })
...
func (s *BackupService) deleteBackupObjects(ctx context.Context, record *BackupRecord) error {
	...
	if s3Cfg == nil || !s3Cfg.IsConfigured() {
		// Old records remain removable after object-storage settings are cleared.
		return nil
	}
	objectStore, err := s.getOrCreateStore(ctx, s3Cfg)
	if err != nil {
