...
func TestEstimateOpenAIAnthropicStreamInputTokensExcludesInlineImagePayload(t *testing.T) {
	payload := []byte(`{"model":"gpt-5.6","input":[{"type":"input_image","image_url":"data:image/png;base64,` + strings.Repeat("A", 1<<20) + `"}]}`)

	estimated := estimateOpenAIAnthropicStreamInputTokens(payload, "gpt-5.6")

	require.Positive(t, estimated)
	require.Less(t, estimated, 100, "inline base64 bytes must not masquerade as text tokens")
}
