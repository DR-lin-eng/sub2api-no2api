...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	if shouldMimicClaudeCode {
		...
		// Preserve client-owned system cache_control. The final cap is applied
		// below by enforceCacheControlLimit, so rolling upgrades keep existing
		// prompt-cache anchors without exceeding the upstream four-breakpoint cap.
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		// 发送请求
		resp, err = doAccountHTTPUpstreamWithTLS(s.httpUpstream, upstreamReq, proxyURL, account, tlsProfile)
		if err != nil {
		...
		if resp.StatusCode == 400 && !distillation {
			...
			if readErr == nil {
				...
				if !distillation && s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
					...
					releaseRetryCtx()
					if buildErr == nil {
						retryResp, retryErr := doAccountHTTPUpstreamWithTLS(s.httpUpstream, retryReq, proxyURL, account, tlsProfile)
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isSignatureErrorPattern(ctx, account, retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									releaseRetryCtx2()
									if buildErr2 == nil {
										retryResp2, retryErr2 := doAccountHTTPUpstreamWithTLS(s.httpUpstream, retryReq2, proxyURL, account, tlsProfile)
										if retryErr2 == nil {
				...
				if !distillation && isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						if buildErr == nil {
							budgetRetryResp, retryErr := doAccountHTTPUpstreamWithTLS(s.httpUpstream, budgetRetryReq, proxyURL, account, tlsProfile)
							if retryErr == nil {
	...
	if reqStream {
		...
		if err != nil {
			...
			if errors.As(err, &sseErr) {
				// 上游 HTTP 200 + SSE 流体内出现 event:error 帧。若错误是
				// overloaded_error 且尚未向客户端写出内容，按 529 走标准
				// failover；已经写出内容时保持 403 语义，避免破坏流协议。
...
func billingModelForRestriction(source, requestedModel, channelMappedModel string) string {
	...
	case BillingModelSourceResponse:
		// The response is unavailable during dispatch. Use the mapped model for
		// restriction checks, then decide the billing basis after the response.
