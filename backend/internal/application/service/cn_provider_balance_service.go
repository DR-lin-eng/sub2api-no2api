...
func (s *CNProviderBalanceService) queryBalanceForAccount(ctx context.Context, account *Account) (*CNProviderBalanceResult, error) {
	...
	if targetURL == "" {
		return nil, infraerrors.New(http.StatusBadRequest, "CN_BALANCE_NO_ENDPOINT", "account provider has no balance endpoint for its configured base url")
	}
	...
	resp, err := doAccountHTTPUpstreamWithConcurrency(s.httpUpstream, req, proxyURL, account, maxInt(account.Concurrency, 1))
	if err != nil {
...
//
//   - Kimi：仅官方 api.moonshot.cn 账号使用固定余额端点；中转账号不暴露该端点
...
func cnBalanceURL(account *Account) string {
	...
	case PlatformKimi:
		...
		if err != nil || !strings.EqualFold(base.Hostname(), "api.moonshot.cn") {
			return ""
		}
