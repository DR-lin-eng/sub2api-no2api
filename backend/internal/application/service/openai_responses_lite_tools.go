...
func ensureOpenAIResponsesLiteParallelToolCalls(reqBody map[string]any, changed bool) (bool, error) {
	if reqBody == nil {
		return false, newOpenAIResponsesLiteValidationError("body", "responses Lite requires a JSON object")
	}
...
// normalizeOpenAIResponsesLitePayloadForAccount keeps the namespace-tool
// conversion for OAuth/SetupToken accounts and applies only the Lite parallel
// call contract to API-key accounts. This avoids sending an OAuth-only
// additional_tools carrier to ordinary OpenAI-compatible upstreams.
