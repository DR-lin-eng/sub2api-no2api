...
// xAI Imagine accepts a small fixed set of aspect ratios. Keep this list
// local to the adapter so OpenAI size parsing does not leak into billing rules.
...
func applyGrokImagineImageGeometry(body []byte) ([]byte, error) {
	...
	resolution := normalizeGrokImagineImageResolution(gjson.GetBytes(body, "resolution").String())
...
func assignGrokMediaResolution(value string, info *GrokMediaRequestInfo) {
	...
	if imageResolution := normalizeGrokImagineImageResolution(value); imageResolution != "" {
...
func normalizeGrokImagineImageResolution(value string) string {
...
func grokImagineImageResolutionFromSize(size string) string {
	if explicit := normalizeGrokImagineImageResolution(size); explicit != "" {
		return explicit
	}
