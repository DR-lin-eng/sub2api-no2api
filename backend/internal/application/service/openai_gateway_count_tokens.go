...
// ForwardResponsesInputTokens handles native OpenAI POST
// /v1/responses/input_tokens requests without entering the generation or usage
// recording pipeline. Custom relays commonly expose /responses but not this
// preflight endpoint, so those accounts use the bounded local estimator.
func (s *OpenAIGatewayService) ForwardResponsesInputTokens(
	...
	if err != nil {
		...
		return fmt.Errorf("responses input_tokens: read response: %w", err)
	}
	...
	if !gjson.GetBytes(respBody, "input_tokens").Exists() {
		...
		return fmt.Errorf("responses input_tokens: response missing input_tokens")
	}
...
func writeOpenAIResponsesInputTokensFallback(c *gin.Context, account *Account, prepared *openAIInputTokensCountPrepared, statusCode int, reason string) {
	...
	logger.L().Info("openai responses input_tokens: local estimate fallback",
		zap.Int64("account_id", accountID), zap.Int("upstream_status", statusCode),
		zap.Int("estimated_input_tokens", estimated), zap.String("upstream_model", upstreamModel),
