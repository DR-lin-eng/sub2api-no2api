...
func TestCodexFullSimulationIngressUsesOneAttemptPlanForHandshakeAndBody(t *testing.T) {
	...
	require.Equal(t, headers.Get("x-codex-installation-id"), gjson.GetBytes(written, "client_metadata.x-codex-installation-id").String())
	...
	require.True(t, gjson.GetBytes(written, "client_metadata.preserved").Bool())
