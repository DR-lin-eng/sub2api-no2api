...
func (s *OpenAIGatewayService) handleChatBufferedStreamingResponse(
	...
	finalResponse, usage, acc, err := s.readOpenAICompatBufferedTerminal(resp, "openai chat_completions buffered", requestID, c)
	if err != nil {
...
func (s *OpenAIGatewayService) newOpenAICompatBufferedReadFailoverError(
	...
	// Keep the stable code in the replay envelope so exhausted failover and
	// error-passthrough rules can still distinguish transport interruptions.
