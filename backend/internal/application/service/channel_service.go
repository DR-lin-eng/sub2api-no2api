...
type ChannelService struct {
	...
	stopCacheUpdates func()
	stopCacheOnce    sync.Once
}
...
func (s *ChannelService) loadCache(ctx context.Context) (*channelCache, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		result, err, _ := s.cacheSF.Do("channel_cache", func() (any, error) {
			if cached, ok := s.cache.Load().(*channelCache); ok && cached != nil && time.Since(cached.loadedAt) < channelCacheTTL {
				return cached, nil
			}
			return s.buildCache(ctx)
		})
		if errors.Is(err, errChannelCacheInvalidated) {
			continue
		}
		if err != nil {
			return nil, err
	...
	return nil, errChannelCacheInvalidated
}
...
var errChannelCacheInvalidated = errors.New("channel cache changed during reload")

// buildCache publishes only if no update arrived while reading PostgreSQL.
// Gateway cache hits still use the immutable atomic snapshot without locking.
...
// InvalidateCache invalidates the platform-aware pricing/mapping snapshot.
// It is exported through a narrow application interface for group updates.
func (s *ChannelService) InvalidateCache() {
	s.invalidateCache()
}
...
func validatePricingEntries(pricing []ChannelModelPricing) error {
	...
	for _, entry := range pricing {
		if err := ValidateChannelTimePricing(entry.TimePricing); err != nil {
			return infraerrors.BadRequest(
				"INVALID_CHANNEL_TIME_PRICING",
				fmt.Sprintf("invalid time pricing for platform '%s' models %v: %v", entry.Platform, entry.Models, err),
			)
		}
	}
...
// toPricingModelEntry must use exactly the same normalization as channel
// pricing cache keys. Mapping keys intentionally keep their existing rules.
