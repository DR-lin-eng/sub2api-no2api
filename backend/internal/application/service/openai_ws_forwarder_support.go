...
func (s *OpenAIGatewayService) resolveAccountByPreviousResponseIDForCapability(
	...
	if s.schedulerSnapshot != nil && s.accountRepo != nil {
		...
		if !codexContinuationSchedulingMatches(ctx, latest) {
			return 0, nil, "", nil
		}
		...
		if requestedModel != "" && !latest.IsModelSupportedForRequest(requestedModel, openAIRequestModelForSupport(ctx, requestedModel)) {
			return 0, nil, "", nil
		}
