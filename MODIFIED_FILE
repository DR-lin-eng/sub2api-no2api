...
func (a *Account) IsModelSupportedForRequest(requestedModel, routedModel string) bool {
	...
	if len(mapping) == 0 {
		...
		if a.Platform == PlatformDeepseek {
			return isDeepseekServableModel(firstNonEmptyModel(routedModel, requestedModel))
		}
...
func (a *Account) GetOpenAIBaseURL() string {
	if a == nil || (!a.IsOpenAI() && !a.IsCNProvider() && !a.IsOpenCodeGo()) {
		return ""
	}
...
// GetAPIProtocol returns the pinned or adaptive upstream protocol for a
// multi-protocol API-key provider. OpenCode defaults to adaptive routing.
...
func (a *Account) GetOpenAIProtocolAPIKey() string {
	if a == nil || a.Type != AccountTypeAPIKey || (!a.IsOpenAI() && !a.IsMultiProtocolAPIKey()) {
		return ""
	}
	return a.GetCredential("api_key")
}
...
// Anthropic OAuth/SetupToken and OpenAI OAuth (Codex) accounts support this
// account-level forwarding override.  OAuth authorization and refresh
// endpoints remain on their provider-owned URLs; this flag only affects model
// request egress.
...
// The value is deliberately kept separate from credentials so it cannot alter
// OAuth token exchange or refresh destinations.
