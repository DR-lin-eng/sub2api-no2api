...
const hasCanonicalGrokTier = (value: unknown): boolean => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false
  const tier = (value as Record<string, unknown>).subscription_tier
  return typeof tier === 'string' && tier.trim().length > 0
}
...
export const buildGrokUsageRefreshKey = (account: Pick<Account, 'platform' | 'extra'>): string => {
  ...
  return [
    ...
    serializeSnapshotRefreshValue(
      hasCanonicalGrokTier(usageSnapshot) ? undefined : extra.grok_quota_snapshot
    )
  ].join('|')
}
