export default {
  ...
  keys: {
    ...
    groupBindings: {
      label: '密钥分组',
      rateProtection: '倍率上限',
      rateProtectionFor: '{group} 的倍率上限',
      unlimited: '不限制',
      addGroup: '添加分组',
      orderHint: '多个分组包含相同模型时，将按排列顺序依次尝试。倍率超过保护上限的分组会被跳过。',
      subscriptionSingleHint: '订阅分组暂不支持与其他分组组合。',
      dragToReorder: '拖动调整优先级',
      moveUp: '上移',
      moveDown: '下移',
      removeGroup: '移除分组',
      invalidRateProtection: '倍率上限必须为空或大于 0',
      primary: '首选',
      fallback: '备用',
      fallbackPosition: '第 {position} 备用',
      singleGroup: '未配置备用分组',
      notConfigured: '未配置分组路由',
      configure: '立即配置',
      manage: '管理',
      manageFor: '管理 {name} 的分组路由',
      manageEmpty: '尚未配置分组路由',
      moreGroups: '+{count}',
      rateCeilingShort: '≤{rate}x',
      dialogTitle: '管理分组路由',
      dialogDescription: '为「{name}」配置路由优先级和可接受的倍率上限。',
      groupCount: '已配置 {count} 个分组',
      currentPrimary: '当前首选：{group}',
      save: '保存分组',
      updatedSuccess: '分组路由已更新',
      updateFailed: '更新分组路由失败',
    },
    selectGroup: '选择分组',
    statusLabel: '状态',
    selectStatus: '选择状态',
  ...
  usage: {
    ...
    timingComparison: '计时对比',
    timingLocal: '本地',
    timingFirstSource: '展示首字来源',
    timingDurationSource: '展示耗时来源',
    timingLocalFirst: '本地首字',
    timingLocalDuration: '本地总耗时',
    timingEngineFirst: 'OpenAI 引擎首字',
    timingUpstreamDuration: 'OpenAI 轮次耗时',
    timingFirstDifference: '首字差值（本地 - OpenAI）',
    timingDurationDifference: '耗时差值（本地 - OpenAI）',
    timingSampledFirst: 'OpenAI 采样首字',
    timingQueue: 'OpenAI 最长排队',
    timingAPIDuration: 'OpenAI API 耗时（不含客户端工具）',
    timingEngineCalls: 'OpenAI 引擎调用数',
