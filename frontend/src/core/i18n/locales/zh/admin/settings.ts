...
export default {
    settings: {
      ...
      permissionGroups: {
        catalog: {
          support_read: { name: "在线客服：查看", description: "查看客服会话、消息和未读计数" },
          support_write: { name: "在线客服：处理", description: "回复、上传、撤回、标记会话和管理素材" },
          support_balance_transfer: { name: "在线客服：余额转账", description: "通过客服会话向用户执行余额转账" },
          users_read_basic: { name: "用户：基本信息", description: "通过客服会话查看基本资料，不含密钥和登录身份" },
          users_manage: { name: "用户：管理", description: "列出、创建、编辑和删除普通用户；敏感字段需额外权限" },
        ...
        title: '权限组',
        description: '在这里创建自定义角色，控制管理员可以查看和操作的功能。客服组默认保留，并包含在线客服和用户基本信息权限。',
        add: '新增权限组',
        name: '组名称',
        permissions: '权限',
        builtIn: '内置权限组',
      ...
      streamTimeout: {
        ...
        openAIStartupTitle: 'OpenAI Responses 首输出保护',
        openAIStartupDescription: '首个语义输出前保留可重放内容，并在切号时维持同一条客户端连接。设为 0 可关闭对应限制。',
        firstOutputTimeout: '首个语义输出（秒）',
        firstOutputTimeoutHint: '普通请求默认 90 秒；设为 0 关闭截止时间。',
        highEffortFirstOutputTimeout: '高推理输出（秒）',
        highEffortFirstOutputTimeoutHint: '用于 high、xhigh、max 推理；设为 0 沿用普通值。',
        keepaliveInterval: 'SSE 保活间隔（秒）',
        keepaliveIntervalHint: '等待或切号期间发送注释心跳；设为 0 关闭保活。',
