export default {
  promptAudit: {
    ...
    common: { actions: '操作', never: '从未', unknown: '未知' },
    mode: { off: '已关闭', async_audit: '异步只审计', blocking: '同步审计并阻止', unknown: '未知' },
    status: { disabled: '未启用', running: '运行中', degraded: '降级', error: '错误', healthy: '健康', failed: '失败', stale: '心跳过期', unknown: '未知' },
    ...
    runtime: {
      ...
      queue: '活动任务 / 容量', dependencies: '依赖', dependenciesValue: 'DB {database} · Redis {redis}', guardMetrics: '同步 Guard 指标', latest: '最近处理与错误',
      dependencyStatus: { ok: '正常', error: '错误', unavailable: '不可用', unknown: '未知' },
