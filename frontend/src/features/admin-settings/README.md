...
- `presentation/widgets/settings-tabs/gateway-resilience/`: Codex A/B、系统强制账号预热、临时不可调度、OAuth 401 自动清理、冷却、流超时、请求修正与策略设置卡片；直接复用页面 context，由网关韧性 panel 按原顺序装配。Codex 查询失败时不得显示默认关闭状态，独立强制恢复 action 必须保持可用。
