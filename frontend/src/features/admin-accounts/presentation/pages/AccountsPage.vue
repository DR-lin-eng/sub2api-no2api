<template>
  <AppLayout>
    ...
    <AccountActionMenu :show="menu.show" :account="menu.acc" :anchor-rect="menu.anchorRect" @close="menu.show = false" @test="handleTest" @sync-cpa="handleSyncCPA" @stats="handleViewStats" @schedule="handleSchedule" @duplicate="handleDuplicateAccount" @reauth="handleReAuth" @refresh-token="handleRefresh" @recover-state="handleRecoverState" @reset-quota="handleResetQuota" @set-privacy="handleSetPrivacy" @create-spark-shadow="handleCreateSparkShadow" />
...
const openMenu = (a: Account, e: MouseEvent) => {
  ...
  menu.anchorRect = target?.getBoundingClientRect() ?? new DOMRect(e.clientX, e.clientY, 0, 0)
  menu.show = true
}
...
const handleBulkRefreshToken = async () => {
  ...
  const accountIds = [...selIds.value]
  try {
    const result = await accountActions.batchRefresh(accountIds)
    if (result.failed > 0) {
      appStore.showError(t('admin.accounts.bulkActions.partialSuccess', { success: result.success, failed: result.failed }))
      const failedIds = result.errors?.map((error) => error.account_id) ?? result.failed_ids ?? []
