<template>
  <AppLayout>
    ...
    >
      <form id="activity-center-form" @submit.prevent="handleSave" class="space-y-3">
        ...
        <div v-else-if="form.type === 'checkin'" class="rounded-lg border border-gray-200 p-3 dark:border-dark-600">
          ...
          <div class="mt-3"><label class="input-label">{{ t('admin.activityCenter.config.dailyRewards') }}</label><div v-for="reward in checkinConfig.daily_rewards" :key="reward.day" class="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-5"><input :value="reward.day" readonly type="number" min="1" class="input" :placeholder="t('admin.activityCenter.config.rewardDay')" /><select v-model="reward.reward_type" class="input"><option value="balance">{{ t('admin.activityCenter.config.balance') }}</option><option value="concurrency">{{ t('admin.activityCenter.config.concurrency') }}</option><option value="subscription">{{ t('admin.activityCenter.config.subscription') }}</option></select><input v-model="reward.value" class="input" :placeholder="reward.reward_type === 'subscription' ? t('admin.activityCenter.config.subscriptionDays') : reward.reward_type === 'concurrency' ? t('admin.activityCenter.config.concurrencyCount') : t('admin.activityCenter.config.balanceAmount')" /><input v-model="reward.label" class="input" :placeholder="t('admin.activityCenter.config.rewardLabel')" /><Select v-if="reward.reward_type === 'subscription'" v-model="reward.reward_group_id" :options="rewardGroupOptions" :placeholder="t('admin.activityCenter.config.selectSubscriptionGroup')" searchable /></div></div>
    ...
    <ActivityCardCodeDialog
      :show="activeCodePrize !== null"
      :prize-id="activeCodePrize?.id || ''"
      :label="activeCodePrize?.label || ''"
      :codes-text="activeCodePrize?.codes_text || ''"
      :status-class="activeCodePrize ? cardCodesClass(activeCodePrize) : ''"
      :status-text="activeCodePrize ? cardCodesSummary(activeCodePrize) : ''"
      @update:codes-text="value => { if (activeCodePrize) activeCodePrize.codes_text = value }"
...
import ActivityCardCodeDialog from '../widgets/ActivityCardCodeDialog.vue'
import { completeCheckinCalendar } from '../checkinCalendar'
...
const checkinConfig = reactive({
  ...
  required_group_ids: [] as number[],
  daily_rewards: completeCheckinCalendar('weekly')
})
...
function resetActivityConfig() {
  ...
  checkinConfig.daily_rewards = completeCheckinCalendar('weekly')
}
...
function openCardCodeDialog(prize: LotteryPrizeForm) { activeCodePrize.value = prize }
...
async function handleSave() {
  ...
  try {
    ...
    const payload = {
      ...
      banner_url: editingCampaign.value?.banner_url || '',
      banner_html: form.banner_html,
      type: form.type,
      ref_id: editingCampaign.value?.ref_id || '',
...
watch(() => checkinConfig.cycle_type, cycle => { checkinConfig.daily_rewards = completeCheckinCalendar(cycle, checkinConfig.daily_rewards) })
