      ...
      >
        ...
        >
          ...
          <div v-if="message.role === 'assistant'" class="mt-3">
            ...
            <div v-if="message.status === 'processing' || message.status === 'queued'" class="grid gap-3 sm:grid-cols-2">
              <div
                v-for="index in (message.count || 1)"
                :key="index"
                class="aspect-square animate-pulse rounded-2xl border border-gray-200 bg-gray-100 dark:border-dark-700 dark:bg-dark-800"
              ></div>
            </div>

            <div v-else-if="message.status === 'completed' && message.images?.length" class="grid gap-3 sm:grid-cols-2">
              <a
              ...
              >
                <img :src="image.src" :alt="image.revisedPrompt || message.prompt" class="aspect-square w-full object-cover transition duration-200 group-hover:scale-[1.02]" />
              </a>
            </div>
            ...
            >
              {{ t('mediaStudio.session.noImageResult') }}
            </div>
          </div>
        </div>
      </article>
        ...
        <div class="relative mx-auto max-w-4xl">
          ...
          <div class="rounded-[1.65rem] border border-gray-200 bg-white/92 p-3 shadow-[0_16px_48px_rgba(15,23,42,0.07)] backdrop-blur dark:border-dark-700 dark:bg-dark-900/90 dark:shadow-black/20">
            <textarea
              ...
              :placeholder="t('mediaStudio.composer.placeholder')"
            ...
            <div v-if="!loadingKeys && apiKeys.length === 0" class="mt-2 flex items-center gap-2 px-1 text-xs text-gray-500 dark:text-gray-400">
