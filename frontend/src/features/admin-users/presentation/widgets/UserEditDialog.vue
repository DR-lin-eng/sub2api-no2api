  ...
  >
    <form v-if="user" id="edit-user-form" @submit.prevent="handleUpdateUser" class="space-y-5">
      <div>
        ...
        <input v-model="form.email" :disabled="!authStore.hasPermission('users.credentials')" type="email" class="input" />
      </div>
      <div>
        <label class="input-label">{{ t('admin.users.password') }}</label>
        <div class="flex gap-2">
          <div class="relative flex-1">
            <input v-model="form.password" :disabled="!authStore.hasPermission('users.credentials')" type="text" class="input pr-10" :placeholder="t('admin.users.enterNewPassword')" />
          ...
          <button type="button" :disabled="!authStore.hasPermission('users.credentials')" @click="generatePassword" class="btn btn-secondary px-3">
...
import { listPermissionGroups, update as updateAdminUser } from '@/features/admin-users/data/datasources/adminUsersDatasource'
...
watch(() => props.show, (show) => { if (show && authStore.isAdmin) listPermissionGroups().then((data) => { permissionGroups.value = data.groups }).catch(() => { permissionGroups.value = [] }) }, { immediate: true })
...
const handleUpdateUser = async () => {
  ...
  try {
    ...
    if (!authStore.hasPermission('users.credentials')) delete data.email
    if (!authStore.hasPermission('users.billing')) { delete data.concurrency; delete data.rpm_limit; delete data.scheduling_tier }
