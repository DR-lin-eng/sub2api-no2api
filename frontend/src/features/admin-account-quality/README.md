...
质量设置使用 `account_quality_settings`，运行状态使用 `account_quality_state`；`max_concurrent` 默认 4，可由管理员设置到 200；质量巡检仅检测状态启用且已启用调度（`schedulable=true`）的 OpenAI/Gemini OAuth 账号，不检测 API Key、service account 或关闭调度的账号。配置检测源分组后，检测范围同时包含源分组、降智分组和旧配置已迁出的账号；连续恢复达到阈值后，有原分组标记的账号恢复原绑定，无标记且仍仅绑定降智分组的账号回到配置的源分组，其他人工分组变更保持不动。OpenAI OAuth 探测会记录通过阶段响应中的 `X-Codex-Turn-State`，结果表分别显示采集值和实际注入值；`inject_turn_state` 可独立打开，按账号从网关控制池随机注入，用于开启/关闭对照。原始 state 不进入公开质量页面。流式画图内容不完整时仍保留代码匹配结果并标记“输出被中断，分析可能错误”；账号总结果以文字题为准，完全空的失败记录不进入公开面板。上一轮未完成时，新的定时或手动轮次合并为一个待开始任务，当前轮次不会被中断；上一轮完成后立即启动排队轮次。账号健康巡检使用另一组设置、状态和调度器。
