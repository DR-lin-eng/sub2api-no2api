  ...
  <AppLayout>
    <main class="mx-auto max-w-[1400px] space-y-5 pb-12">
      ...
      <section v-if="run?.progress" class="rounded-lg border border-primary-200 bg-primary-50 p-4 dark:border-primary-900/60 dark:bg-primary-950/30">
        ...
        <p class="mt-2 text-xs text-primary-800 dark:text-primary-200">{{ t('admin.accountQuality.progressCounts', { queued: runProgress.queued || 0, running: runProgress.running || 0 }) }} · {{ t('admin.accountQuality.pendingRounds', { count: runProgress.pending_runs || 0 }) }} · {{ t('admin.accountQuality.elapsed') }} {{ elapsed(run?.started_at, run?.completed_at) }}</p>
      </section>
      ...
      <section class="border-y border-gray-200 py-5 dark:border-dark-700">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label class="toggle-field"><span>{{ t('admin.accountQuality.injectTurnState') }}</span><Toggle :model-value="settings.inject_turn_state" data-testid="quality-inject-turn-state" @update:model-value="settings.inject_turn_state = $event" /></label>
          ...
          <label class="field-group"><span class="field-label">{{ t('admin.accountInspection.quality.maxConcurrent') }}</span><input v-model.number="settings.max_concurrent" class="input" type="number" min="1" max="200" /><span class="field-hint">{{ t('admin.accountInspection.quality.maxConcurrentHint') }}</span></label>
      ...
      <section class="overflow-x-auto rounded-lg border border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-800">
        <table class="w-full min-w-[1450px] text-left text-sm">
          <thead class="bg-gray-50 text-xs text-gray-500 dark:bg-dark-900/60"><tr><th class="px-4 py-3">{{ t('admin.accountInspection.results.account') }}</th><th class="px-4 py-3">{{ t('admin.accountQuality.stageResults') }}</th><th class="px-4 py-3">{{ t('admin.accountInspection.results.state') }}</th><th class="px-4 py-3">{{ t('admin.accountQuality.reasoningTokens') }}</th><th class="px-4 py-3">{{ t('admin.accountQuality.turnState') }}</th><th class="px-4 py-3">{{ t('admin.accountInspection.results.action') }}</th><th class="px-4 py-3">{{ t('admin.accountInspection.results.reason') }}</th><th class="px-4 py-3">{{ t('admin.accountQuality.probeLatency') }}</th></tr></thead>
          <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
            <tr v-for="row in results" :key="row.account_id">
              <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ row.name }} <span class="text-xs text-gray-400">#{{ row.account_id }} · {{ row.platform }}</span></td>
              <td class="px-4 py-3 text-xs"><p class="mb-2 text-primary-600 dark:text-primary-400">{{ phaseLabel(row.quality_phase) }} · {{ elapsed(row.quality_started_at, row.quality_completed_at) }}</p><span class="mr-3">S1 {{ qualityStageLabel(row.quality_stage1_status) }}</span><span>S2 {{ qualityStageLabel(row.quality_stage2_status) }}</span><p v-if="row.quality_code_match" class="mt-2 text-gray-500">{{ t('admin.accountQuality.codeMatchScore', { score: row.quality_code_match.score, threshold: row.quality_code_match.threshold }) }}</p></td>
              <td class="px-4 py-3"><span :class="qualityStatusClass(row.quality_status)">{{ qualityStatusLabel(row.quality_status) }}</span><span class="ml-2 text-xs text-gray-400">F{{ row.quality_consecutive_failures || 0 }} / P{{ row.quality_consecutive_passes || 0 }}</span></td>
              <td class="px-4 py-3 tabular-nums">{{ row.quality_reasoning_tokens == null ? t('common.unknown') : row.quality_reasoning_tokens.toLocaleString() }}</td>
              <td class="w-80 px-4 py-3 text-xs">
                <div v-if="row.quality_turn_states?.length" class="space-y-1.5" data-testid="quality-captured-turn-states"><span class="font-medium text-emerald-700 dark:text-emerald-400">{{ t('admin.accountQuality.turnStateCaptured') }}</span><code v-for="state in row.quality_turn_states" :key="`captured-${state}`" class="block max-h-20 overflow-auto whitespace-pre-wrap break-all rounded bg-gray-50 p-1.5 text-[11px] text-gray-700 dark:bg-dark-900 dark:text-gray-300">{{ state }}</code></div>
                <div v-if="row.quality_injected_turn_states?.length" class="mt-2 space-y-1.5" data-testid="quality-injected-turn-states"><span class="font-medium text-primary-700 dark:text-primary-400">{{ t('admin.accountQuality.turnStateInjected') }}</span><code v-for="state in row.quality_injected_turn_states" :key="`injected-${state}`" class="block max-h-20 overflow-auto whitespace-pre-wrap break-all rounded bg-gray-50 p-1.5 text-[11px] text-gray-700 dark:bg-dark-900 dark:text-gray-300">{{ state }}</code></div>
                <span v-if="!row.quality_turn_states?.length && !row.quality_injected_turn_states?.length" class="text-gray-400">-</span>
              </td>
              <td class="px-4 py-3 text-xs">{{ row.quality_action || '-' }}</td><td class="max-w-md truncate px-4 py-3 text-xs text-gray-500" :title="row.quality_error">{{ row.quality_error || row.quality_label || '-' }}</td><td class="px-4 py-3 tabular-nums">{{ row.quality_latency_ms ? `${Math.round(row.quality_latency_ms)}ms` : '-' }}</td>
            </tr>
            <tr v-if="!loading && !results.length"><td colspan="8" class="px-4 py-10 text-center text-gray-500">{{ run?.status === 'running' ? t('admin.accountQuality.progressPreparing') : t('admin.accountInspection.results.noResults') }}</td></tr>
          </tbody>
        </table>
      </section>
...
function qualityStageLabel(status?: string): string { if (status === 'running') return t('admin.accountInspection.running'); if (status === 'queued') return t('admin.accountQuality.queued'); if (status === 'passed') return t('admin.accountQuality.stagePassed'); if (status === 'wrong') return t('admin.accountQuality.stageWrong'); if (status === 'interrupted') return t('admin.accountQuality.outputInterrupted'); if (status === 'uncertain') return t('admin.accountQuality.uncertain'); if (status === 'error') return t('admin.accountQuality.errors'); if (status === 'disabled') return t('admin.accountQuality.stageDisabled'); return '-' }
