...
管理员可以在分组编辑器中启用“可用模型”并选择模型。设置保存在 `groups.model_allowlist`，同时过滤模型列表、Codex manifest、Gemini/Antigravity 列表和批量生图列表，并在网关请求进入合成路由与调度前校验客户端模型名；未启用时保持账号能力发现和旧 `models_list_config` 展示配置兼容。
