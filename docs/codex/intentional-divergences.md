...
共享缓存；日志只记录长度、命中、新旧与代理布尔值。首次缺少正确值时立即探测，已有正确值 45 分钟没有更新时
刷新探测；事件唤醒配合 5 秒扫描，单实例并发 32 个账号/模型目标，每个目标并行竞速最多 4 个代理且每个最多
15 秒，首个成功会取消其余尝试，全部失败后 30 秒重试。探测不写用户用量日志，多实例以共享锁避免重复探测。
