...
| 新增账号巡检 | `application/service/account_inspection.go`, `transport/http/handler/admin/account_inspection_handler.go` | `usage_log_repo_stats.go`, `cmd/server/wire.go`, `frontend/src/features/admin-account-inspection/` | service/handler + repository hourly stats + feature Vitest |
...
| CPA OAuth 账号同步 | `application/service/cpa_sync_service.go`, `modules/cpasync/` | `transport/http/handler/admin/account_cpa_sync.go`, `server/routes/admin.go`, `frontend/src/features/admin-accounts/` | CPA client/service/handler + admin-accounts Vitest/typecheck + Docker |
