...
| 修改账号 IPv6 出口 | `modules/egress/`, `platform/egress/` | `repository/egress_repo.go`, `http_upstream.go`, `features/admin-egress/`, `deploy/docker-compose.ipv6-egress*.yml`, `deploy/ipv6-egress-sidecar.sh` | route/dialer + runtime switch/auto-detect + repository integration + Docker IPv6/6in4 sidecar + 前端 feature 测试 |
