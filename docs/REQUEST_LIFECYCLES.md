...
账号/模型目标。探测使用 `generate=false`、空 input 的 WSv2 零输出 ping，不写用户用量日志；每个目标从当前
活跃且未过期、非 unhealthy 的代理按 ID 稳定排序后轮转选择最多 4 个并行竞速，因此单实例最多同时存在
128 个代理尝试。每个尝试最多 15 秒，取得首个新且正确长度的 state 后取消同目标的其余尝试，全部失败后
30 秒再试。到期目标按 `next_probe_at` 最早优先，多实例通过共享 leader lock 避免同一账号/模型重复探测；探测得到
