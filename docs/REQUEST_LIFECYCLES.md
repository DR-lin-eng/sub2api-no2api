...
`GET /api/v1/account-quality-share`，按 manxue.ai 风格展示 24 小时摘要、可点击状态时间线、检测对话/响应 ID、两阶段最终回答文本、reasoning token 和图片预览；回答只保留最多 16 KiB 并在前端按纯文本渲染，不返回账号 ID、凭据或原始请求。公开展示页允许第三方 HTTPS 站点通过 iframe 嵌入：安全头中间件只对精确路径 `/monitor/quality/public` 省略 `X-Frame-Options` 并把 CSP `frame-ancestors` 设为 `*`，其他页面仍维持全局禁止嵌入策略。图片由
