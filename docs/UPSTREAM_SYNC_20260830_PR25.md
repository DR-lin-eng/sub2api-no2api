...
- Support-chat batches upload at most two images concurrently, matching the backend's two bounded image-decode slots while preserving the user's selection order. A four-image 4000x4000 browser reproduction changed from two `503` responses to four successful uploads and one structured message.
